 ...
 * its own copy). A pure classifier: a small thing → a clean task object; a project → a
 * `route` hand-off to the Project agent (it does NOT ask the project's questions itself).
 * It NEVER computes a date — a spoken time is copied verbatim into timeContext.
 */
...
If it is a project-sized goal (multiple steps / a real workflow), do NOT ask anything and do
NOT plan — just hand it off. Return exactly:
